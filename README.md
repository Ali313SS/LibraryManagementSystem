# 📖 Library Management System

![Java](https://img.shields.io/badge/Java-17%2B-blue)
![MSSQL](https://img.shields.io/badge/MySQL-8.0-orange)
![Maven](https://img.shields.io/badge/Maven-3.8%2B-red)

A desktop application for managing library operations, built with Java and MySQL. Designed for librarians to efficiently track books, members, and transactions.

[![GitHub Stars](https://img.shields.io/github/stars/Ali313SS/LibraryManagementSystem?style=social)](https://github.com/Ali313SS/LibraryManagementSystem/stargazers)

## ✨ Features
- **Admin Dashboard** with system overview
- **Book Management** (Add/Edit/Delete books)
- **Member Registration** with unique IDs
- **Transaction System** for book borrowing/returns
- **Search Functionality** for books/members
- **Overdue Book Tracking**
- **Database Backup & Restore**

## 🛠 Tech Stack
- **Core Language**: Java 17
- **GUI**: JavaFX
- **Database**: MSSQL
- **Build Tool**: Maven
- **Dependency Management**: JDBC

## 🚀 Getting Started

### Prerequisites
- Java 
- MSSQL 
- Maven 

### Installation
1. **Clone the repository**:
```bash
git clone https://github.com/Ali313SS/LibraryManagementSystem.git
cd LibraryManagementSystem
